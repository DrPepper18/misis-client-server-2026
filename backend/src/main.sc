...
theme: /
    ...
    state: Help
        ...
        a: Доступные города: Санкт-Петербург, Сочи, Адлер, Казань, Екатеринбург, Владивосток, Ростов-на-Дону, Калининград, Новосибирск. Выбирайте, что вам по душе!
    ...
    state: PlanTrip
        ...
        script:
            ...
            if (city.toLowerCase() == "ростов" || city.toLowerCase() == "дону") {
                city = "Ростов-на-Дону";
            } else {
        ...
        if: $temp.success
            ...
            a: К сожалению, под ваш запрос ничего не нашлось. Попробуйте поменять сумму или даты.

    
    state: HandleReturn
        event: USER_RETURNED
        script:
            $context.response.replies = []; // Обнуляем то, что бот успел наговорить


    state: Fallback
        event!: noMatch
        a: Я не совсем понял запрос. Скажите, например: "Хочу тур в Сочи 10 июня за 50000 рублей".
